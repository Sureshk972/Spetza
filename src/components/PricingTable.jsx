...
export default function PricingTable({ variant = 'sender', collapsible = false }) {
  const [open, setOpen] = useState(!collapsible)

  const header = (
    <div
      className={`text-xs uppercase tracking-widest text-slate ${collapsible ? 'flex items-center justify-between cursor-pointer select-none' : 'mb-3'}`}
      onClick={collapsible ? () => setOpen((o) => !o) : undefined}
    >
      <span>Pricing</span>
      {collapsible && <span className="text-sm">{open ? '▲' : '▼'}</span>}
    </div>
  )
