...
Deno.serve(async (req) => {
  ...
  if (profile?.account_type !== "sender" || !profile.stripe_customer_id) {
    return json({ error: "sender profile not set up" }, 409);
  }
  ...
  await supabase
    .from("profiles")
    .update({ stripe_default_payment_method_id: payment_method_id })
    .eq("id", user.id);
