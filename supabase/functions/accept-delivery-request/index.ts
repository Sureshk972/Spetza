...
Deno.serve(async (req) => {
  ...
  const { delivery_request_id, courier_liability_accepted } = await req.json().catch(() => ({}));
  if (!delivery_request_id) return json({ error: "missing delivery_request_id" }, 400);
  if (!courier_liability_accepted) return json({ error: "liability acknowledgment required" }, 400);
